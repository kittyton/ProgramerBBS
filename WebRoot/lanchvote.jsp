...
function add(){  
   ...
   //增加行元素  
   var inputcell2_1 = document.createElement("<input size='32' name=''>");  
      cell2.appendChild(inputcell2_1);  
   var inputcell2_2 = document.createElement("<input type='button' value='删除选项'onClick='del(this);'/>");  
...
function frash(){  
   ...
   var n = num;  
      for(i=3;i<n-1;i++){  
    //设置标签值  
       table.rows[i].cells[0].innerText="选项"+(i-2);  
       //设置属性值  
    table.rows[i].cells[1].childNodes[0].setAttribute("name","select"+(i-2));  
...
<div>
<form action="Votelanch" method="post">
<table id="table1">  
<tr>  
   <td>投票主题</td>  
   <td><input style="width:8cm;" name="vote_title"></td>
   </tr>
   <tr>  
   <td>投票说明</td>  
   <td><input style="width:8cm;" name="vote_des" type="text"></td>  
</tr> 
<tr>
<td>投票截止日期</td>
<td><input style="width:8cm;" type="text" name="vote_deadline" onfocus="c.showMoreDay = false;c.show(this);" /></td>
</tr> 
<tr>  
   <td style="padding-top:0.3cm;"><a href="#" onclick="add();">增加元素</a></td>  
</tr>  
</table> 
<input style="position:relative;left:9cm;" type="submit" value="发起投票"/>
</form> 
</div>
